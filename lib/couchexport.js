 ...
 * - indexOpts (Object): index retrieval options, format { db: 'databasename', design: 'designdocname', view: 'viewname', fetchSize: 1000000, filter: function (rows) {} }, filter function should construct an array of keys based on CouchDB result rows
 * - docsOpts (Object): docs retrieval options, format { db: 'databasename', design: 'designdocname', view: 'viewname', fetchSize: 1000000, filter: function (row) {} }, if design or view is not supplied then documents will be retrieved from the database, otherwise the view will be used
...
CouchExport.prototype.exportToFile = function (file, decorators, indexOpts, docsOpts, cb) {
  ...
  indexOpts.filter = indexOpts.filter || function (rows) {
    return _.pluck(rows, 'key');
  };
  docsOpts.filter = docsOpts.filter || function (row) {
    return row.doc;
  };
  ...
  function _indexFetchCb(rows, cb) {
    ...
    var keys = indexOpts.filter(rows);

    function _docsCb(rows) {
      rows.forEach(function (row) {
        var doc = docsOpts.filter(row);
        if (doc) {
  ...
  streamer.write(formatter.header());
