 ...
 * - indexOpts (Object): index retrieval options, format { db: 'databasename', design: 'designdocname', view: 'viewname', fetchSize: 1000000, createKeys: function () {} }, createKeys function should construct an array of keys based on CouchDB result rows
 * - docsOpts (Object): docs retrieval options, format { db: 'databasename', design: 'designdocname', view: 'viewname', fetchSize: 1000000 }, if design or view is not supplied then documents will be retrieved from the database, otherwise the view will be used
  ...
  function _indexFetchCb(rows, cb) {
    ...
    function _docsCb(rows) {
      rows.forEach(function (row) {
        streamer.write(formatter.format(row.doc));
      });
    }
